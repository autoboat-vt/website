...
#   1. Build the site with `bun run build` → dist/
#   2. Fetch the latest aoe_sites/main
#   3. Create a worktree based on aoe_sites/main
#   4. Remove all tracked files, copy in dist/ contents, commit
#   5. Push (fast-forward — no force, no orphan branch)
#
...
# --- Step 4: Replace all contents with dist/ --------------------------------
...
# --- Step 5: Push (fast-forward, NO force) ----------------------------------
