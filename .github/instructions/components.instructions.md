...
- **Hub pages**: `src/pages/Index.tsx` is the nav-visible entry point to the feature pages (`/live`, `/calendar`, `/gallery`) that deliberately have no nav slot. It renders one link-card per entry in its local `FEATURE_PAGES` array (`{ to, title, description, icon }`). ⚠️ Because the features are off-nav, this array is the only in-nav path to them — a wrong `to` makes a page reachable only by typing the URL. Add a card whenever you add a non-nav feature page, and keep `src/test/pages/Index.test.tsx` asserting each `href`. Note the card markup is a raw `<Link>` (not `Card`) so the whole tile is clickable; `Card`'s inner `<h3>` would otherwise need its own link.
