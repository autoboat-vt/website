...
- WARNING: Jest's default cache directory resolves under `.vscode/tmp` in this environment, which the terminal sandbox blocks: `EPERM: operation not permitted, realpath '/Users/<you>/.vscode/tmp/...'`. When `bun run test` fails that way, run `npx jest --cache-directory "$TMPDIR/jest-cache"` instead.
